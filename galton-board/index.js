...
const makeTree = (height) => {
    ...
    for (let y = 2; y <= height; y++) {
        for (let x = 1; x <= y; x++) {
            ...
            const max = (y - 1) * y / 2;
            if (parentN[0] < min || parentN[0] > max)
                parents[0] = undefined;
            if (parentN[1] < min || parentN[1] > max)
                parents[1] = undefined;
            if (current)
                continue;
            ...
            const vector = new Vector(parentVector.x - (x === y ? -gap : gap), parentVector.y + gap * 1.75);
    ...
    const onEnd = () => {
        ...
        ball.addAnimation({
            ...
            update(x) {
                ...
                const b = findRoot(p.y - q.y, -2 * (q.x * (p.y - h) - p.x * (q.y - h)), (p.y - h) * (q.x ** 2) - (q.y - h) * (p.x ** 2));
            ...
            onEnd: () => {
                if (!targetNode)
                    return heightMap.set(toX, (heightMap.get(toX) ?? 0) - 1);
