...
;// ./dist/classes/mouse.js

class Mouse {
    static _mousePosition = new Vec2(0, 0);
    ...
    for (let index = 0; index < sections.length; index++) {
        ...
        section.addEventListener("mouseenter", () => {
            ...
            interval = setInterval(() => {
                const absoluteCenter = new Vec2(item.offsetLeft + section.clientWidth / 2, item.offsetTop + section.clientHeight / 2);
                const center = new Vec2(absoluteCenter.x + section.offsetLeft, absoluteCenter.y + section.offsetTop);
                const direction = Vec2.sub(Mouse, absoluteCenter);
                const distance = Math.hypot(direction.x, direction.y);
                const destination = new Vec2(distance > distanceLimit ? direction.x * (distanceLimit / distance) : direction.x, distance > distanceLimit ? direction.y * (distanceLimit / distance) : direction.y);
