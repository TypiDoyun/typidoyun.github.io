...
const moveButtons = Array.from(document.getElementsByClassName("moveButton")) as HTMLElement[];

const setIndex = (value: number) => {
    index = value;

    if (index < 0 || index >= moveButtons.length) return;
...
const connectImage = (isNext: boolean) => {
