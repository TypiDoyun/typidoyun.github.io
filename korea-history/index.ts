...
const isImportant = (content: string) => {
    return /-\(\d+\.(?: \d+\.)?(?: \d+\.)?\)/.test(content);
}

let buffer: string = "";
let sortedContentList: string[] = [];

contentList.sort((a, b) => {
    const matchesA = getDate(a);
    const matchesB = getDate(b);

    if (matchesA === null) return 1;
    if (matchesB === null) return 1;
...
console.log(contentList.map(content => {
    ...
    return `<li>${content.replace(/(-)?(\((\d+\.(?: \d+\.)?(?: \d+\.)?)\))/, (_: string, important: string, group: string) => {
