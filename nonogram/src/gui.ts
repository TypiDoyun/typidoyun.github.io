...
const submit = () => {
    ...
    for (let i = 0; i < rowAnswers.length; i++) {
        if (rowAnswers[i].some((answer, index) => answer !== rowValues[i][index])) success = false;
    }
    for (let i = 0; i < colAnswers.length; i++) {
        if (colAnswers[i].some((answer, index) => answer !== colValues[i][index])) success = false;
    }
    
    if (success) {
        if (timeTicking) record = Date.now();
        timeTicking = false;
...
export const drawBingo = (board: Board) => {
    const rowAnswers = board.getRowAnswers();
    const colAnswers = board.getColAnswers();
    const rowValues = board.getRowElementAnswers();
    const colValues = board.getColElementAnswers();
    ...
    for (let i = 0; i < board.rows; i++) {
        const text = document.querySelector(`.row-${i} .row-answer`) as HTMLSpanElement;
        text.innerHTML = "";
    ...
    for (let j = 0; j < board.cols; j++) {
        const text = document.querySelector(`.row-0 .frame:nth-child(${j + 2}) .col-answer`) as HTMLSpanElement;
        text.innerHTML = "";
...
export const connectBlock = (board: Board, row: number, col: number) => {
...
export const getBlockType = (board: Board, row: number, col: number) => {
    const block = board.element[row][col];
    if (block.classList.contains("empty")) return BlockType.Empty;
    if (block.classList.contains("flag")) return BlockType.Flag;
    if (block.classList.contains("stone")) return BlockType.Stone;
    throw new Error("Invalid block type");
}

export const setBlockType = (board: Board, row: number, col: number, blockType: BlockType) => {
    const block = board.element[row][col];
...
export const clickBlock = (type: "left" | "right", board: Board, row: number, col: number) => {
