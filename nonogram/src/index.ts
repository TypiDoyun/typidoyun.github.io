...
const submitButton = document.getElementById("done") as HTMLButtonElement;
...
const getBlock = (row: number, col: number) => {
    return boardElement.querySelector(`.row-${row} .col-${col}`) as HTMLDivElement | null;
}
...
const submit = () => {
    ...
    for (let i = 0; i < board.rows; i++) {
        ...
        for (let j = 0; j < board.cols; j++) {
            ...
            if (block?.classList.contains("stone")) count++;
...
const setBlock = (block: HTMLDivElement, type: BlockType) => {
    ...
    for (let i = 0; i < board.rows; i++) {
        ...
        const text = document.querySelector(`.row-${i} .row-num`) as HTMLSpanElement;
        text.innerHTML = "";
    ...
    for (let j = 0; j < board.cols; j++) {
        ...
        const text = document.querySelector(`.row-0 .col-${j} + .col-num`) as HTMLSpanElement;
        text.innerHTML = "";
...
const updateBoard = () => {
    ...
    for (const child of [...boardElement.children]) {
        if (child.id === "done") continue;
        if (child.id === "retry") continue;
        if (child.id === "buttons") continue;
    ...
    for (let j = 0; j < board.cols; j++) {
        ...
        let count = 0;
        for (let i = 0; i < board.rows; i++) {
            if (board.board[i][j] === BlockType.Stone) count++;
